...
def draw_tl_center(image_in, center, state):
    ...
    image_width, image_height, _ = image_out.shape
    image_out = cv2.circle(image_out, center, 1, (0, 0, 0), -1)
    image_out = cv2.putText(image_out, '[' + str(center) + ',' + "'" + state + "'" + ']',
                            (center[0] + 50, center[1]), 1, 0.7, (0, 0, 0))
...
def mark_traffic_signs(image_in, signs_dict):
    ...
    for sign in signs_dict:
        image_out = cv2.circle(image_out, signs_dict[sign], 2, (0, 0, 0), -1)
        image_out = cv2.putText(image_out, '[' + str(signs_dict[sign]) + ',' + "'" + sign + "'" + ']',
                                (signs_dict[sign][0]-50, signs_dict[sign][1] + 75), 1, 0.6, (0, 0, 0))
